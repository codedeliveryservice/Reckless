...
impl MovePicker {
    ...
    fn score_noisy(&mut self, td: &ThreadData) {
        ...
        if td.board.in_check() {
            ...
            for entry in self.list.iter_mut() {
                ...
                entry.score =
                    mp3() * captured.value() + td.noisy_history.get(threats, td.board.moved_piece(mv), mv.to(), captured);
            }
        }
    }
    ...
    fn score_quiet(&mut self, td: &ThreadData, ply: isize) {
        ...
        let escape = [0, mp4(), mp5(), mp6(), mp7(), 0];
        ...
        for entry in self.list.iter_mut() {
            ...
            entry.score = td.quiet_history.get(threats, side, mv)
                ...
                + mp8() * td.board.checking_squares(pt).contains(mv.to()) as i32
                - mp9() * threatened[pt].contains(mv.to()) as i32
                + mp10() * offense[pt].contains(mv.to()) as i32;
        }
    }
}
