...
impl MovePicker {
    ...
    pub fn next(&mut self, td: &ThreadData, skip_quiets: bool) -> Option<Move> {
        ...
        if self.stage == Stage::GoodNoisy {
            while !self.list.is_empty() {
                ...
                let threshold = self.threshold.unwrap_or_else(|| -entry.score / 36 + mp2());
    ...
    fn score_noisy(&mut self, td: &ThreadData) {
        ...
        for entry in self.list.iter_mut() {
            ...
            entry.score = mp3() * PIECE_VALUES[captured] / 128
                + mp4() * td.noisy_history.get(threats, td.board.moved_piece(mv), mv.to(), captured) / 1024;
        }
    }
