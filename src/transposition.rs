...
impl TranspositionTable {
    ...
    pub fn write(
        ...
        for candidate in &mut cluster.entries {
            if candidate.key == key || candidate.depth as i32 == 0 {
        ...
        if !force
            && key == entry.key
            && depth + 5 - (depth == 0) as i32 + 2 * tt_pv as i32 <= entry.depth as i32
...
const fn depth_from_tt(entry_depth: u8) -> i32 {
    entry_depth as i32 - 1
}
