...
pub unsafe fn propagate_l1(ft_out: Aligned<[u8; L1_SIZE]>, nnz: &[u16]) -> Aligned<[f32; L2_SIZE * 2]> {
    ...
    for i in (0..L2_SIZE).step_by(simd::F32_LANES) {
        ...
        let squared = simd::min_f32(simd::mul_f32(vector, vector), one);
...
pub unsafe fn propagate_l2(l1_out: Aligned<[f32; L2_SIZE * 2]>) -> Aligned<[f32; L3_SIZE]> {
