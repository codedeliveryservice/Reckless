...
use crate::{
    ...
    types::{ArrayVec, Bitboard, Color, File, Move, MoveKind, Piece, PieceType, Square},
};
...
impl PstAccumulator {
    ...
    pub fn refresh(&mut self, board: &Board, pov: Color, cache: &mut AccumulatorCache) {
        ...
        let entry = &mut cache.entries[pov][(king.file() >= File::E) as usize]
...
fn pst_index(color: Color, piece: PieceType, square: Square, king: Square, pov: Color) -> usize {
    let flip = (7 * ((king.file() >= File::E) as u8)) ^ (56 * (pov as u8));
