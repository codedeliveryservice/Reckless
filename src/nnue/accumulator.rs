use super::{simd, Aligned, L1_SIZE};
use crate::{
    board::Board,
    lookup::attacks,
    nnue::{threats::threat_index, Parameters, BUCKETS, INPUT_BUCKETS},
...
impl ThreatAccumulator {
        ...
        for square in board.occupancies() {
            ...
            for target in threats {
                ...
                if let Some(index) = threat_index(piece, square, attacked, target, mirrored, pov) {
                    unsafe { add1(parameters, &mut self.values[pov], index) }
                }
            }
        }
    ...
    pub fn update(&mut self, parameters: &'static Parameters, prev: &Self, king: Square, pov: Color) {
        ...
        while !adds.is_empty() && !subs.is_empty() {
            ...
            unsafe { add1_sub1(parameters, &mut self.values[pov], add, sub) }
        }

        for &add in adds.iter() {
            unsafe { add1(parameters, &mut self.values[pov], add) }
        }

        for &sub in subs.iter() {
            unsafe { sub1(parameters, &mut self.values[pov], sub) }
        }
