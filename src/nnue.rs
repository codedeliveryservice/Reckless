...
impl Network {
    ...
    fn update_threat_accumulator(&mut self, accurate: usize, board: &Board, pov: Color) {
        ...
        for i in accurate..self.index {
            if let (prev, [current, ..]) = self.threat_stack.split_at_mut(i + 1) {
                current.update(self.params, &prev[i], king, pov);
            }
        }
    }
    ...
    fn output_transformer(&self, board: &Board) -> i32 {
        unsafe {
            ...
            let l1_out = forward::propagate_l1(params, ft_out, &nnz_indexes[..nnz_count]);
            let l2_out = forward::propagate_l2(params, l1_out);
            let l3_out = forward::propagate_l3(params, l2_out);
