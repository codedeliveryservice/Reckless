...
struct InternalState {
    ...
    non_knight_keys: [u64; Color::NUM],
    non_bishop_keys: [u64; Color::NUM],
    non_rook_keys: [u64; Color::NUM],
    non_queen_keys: [u64; Color::NUM],
    en_passant: Square,
    castling: Castling,
    halfmove_clock: u8,
    plies_from_null: i32,
...
impl Board {
    ...
    pub const fn non_knight_key(&self, color: Color) -> u64 {
        self.state.non_knight_keys[color as usize]
    }

    pub const fn non_bishop_key(&self, color: Color) -> u64 {
        self.state.non_bishop_keys[color as usize]
    }

    pub const fn non_rook_key(&self, color: Color) -> u64 {
        self.state.non_rook_keys[color as usize]
    }

    pub const fn non_queen_key(&self, color: Color) -> u64 {
        self.state.non_queen_keys[color as usize]
    }
    ...
    pub fn update_hash(&mut self, piece: Piece, square: Square) {
        ...
        if piece.piece_type() != PieceType::Knight {
            self.state.non_knight_keys[piece.piece_color()] ^= key;
        }

        if piece.piece_type() != PieceType::Bishop {
            self.state.non_bishop_keys[piece.piece_color()] ^= key;
        }

        if piece.piece_type() != PieceType::Rook {
            self.state.non_rook_keys[piece.piece_color()] ^= key;
        }

        if piece.piece_type() != PieceType::Queen {
            self.state.non_queen_keys[piece.piece_color()] ^= key;
        }
    }
    ...
    pub fn update_hash_keys(&mut self) {
        ...
        self.state.non_knight_keys = [0; Color::NUM];
        self.state.non_bishop_keys = [0; Color::NUM];
        self.state.non_rook_keys = [0; Color::NUM];
        self.state.non_queen_keys = [0; Color::NUM];
