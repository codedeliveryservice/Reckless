...
impl QuietHistory {
    pub fn get(&self, threats: Bitboard, stm: Color, mv: Move, gives_check: bool) -> i32 {
        let entry = &self.entries[gives_check as usize][stm][mv.from()][mv.to()];
        (entry.factorizer + entry.bucket(threats, mv)) as i32
    }

    pub fn update(&mut self, threats: Bitboard, stm: Color, mv: Move, gives_check: bool, bonus: i32) {
        let entry = &mut self.entries[gives_check as usize][stm][mv.from()][mv.to()];
