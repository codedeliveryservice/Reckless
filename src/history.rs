...
pub struct LowPlyHistory {
    entries: Box<[[FromToHistory<QuietHistoryEntry>; 2]; 4]>,
}

impl LowPlyHistory {
    pub fn get(&self, threats: Bitboard, stm: Color, mv: Move, ply: usize) -> i32 {
        let entry = &self.entries[ply][stm][mv.from()][mv.to()];
        (entry.factorizer + entry.bucket(threats, mv)) as i32
    }

    pub fn update(&mut self, threats: Bitboard, stm: Color, mv: Move, bonus: i32, ply: usize) {
        let entry = &mut self.entries[ply][stm][mv.from()][mv.to()];
