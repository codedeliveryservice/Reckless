...
pub struct NoisyHistory {
    // [in_check][piece][to][captured_piece_type][to_threatened]
    entries: Box<[PieceToHistory<NoisyHistoryEntry>; 2]>,
}

impl NoisyHistory {
    pub fn get(&self, in_check: bool, threats: Bitboard, piece: Piece, sq: Square, captured: PieceType) -> i32 {
        let entry = &self.entries[in_check as usize][piece][sq];
        (entry.factorizer + entry.bucket(threats, sq, captured)) as i32
    }

    pub fn update(
        &mut self, in_check: bool, threats: Bitboard, piece: Piece, sq: Square, captured: PieceType, bonus: i32,
    ) {
        let entry = &mut self.entries[in_check as usize][piece][sq];
