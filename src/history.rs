...
impl QuietHistory {
    pub fn get(&self, bucket: usize, threats: Bitboard, stm: Color, mv: Move) -> i32 {
        let entry = &self.entries[bucket][stm][mv.from()][mv.to()];
        (entry.factorizer + entry.bucket(threats, mv)) as i32
    }

    pub fn update(&mut self, bucket: usize, threats: Bitboard, stm: Color, mv: Move, bonus: i32) {
        let entry = &mut self.entries[bucket][stm][mv.from()][mv.to()];
