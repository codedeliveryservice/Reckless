...
pub struct NoisyHistory {
    // [is_recapture][piece][to][captured_piece_type][to_threatened]
    entries: Box<[PieceToHistory<NoisyHistoryEntry>; 2]>,
}

impl NoisyHistory {
    pub fn get(&self, threats: Bitboard, piece: Piece, sq: Square, captured: PieceType, is_recapture: bool) -> i32 {
        let entry = &self.entries[is_recapture as usize][piece][sq];
        (entry.factorizer + entry.bucket(threats, sq, captured)) as i32
    }

    pub fn update(
        &mut self, threats: Bitboard, piece: Piece, sq: Square, captured: PieceType, is_recapture: bool, bonus: i32,
    ) {
        let entry = &mut self.entries[is_recapture as usize][piece][sq];
        entry.update_factorizer(bonus);
