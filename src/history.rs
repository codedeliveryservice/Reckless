...
impl QuietHistoryEntry {
    ...
    pub fn bucket(&self, in_check: bool, threats: Bitboard, mv: Move) -> i16 {
        let from_threated = threats.contains(mv.from()) as usize;
        let to_threated = threats.contains(mv.to()) as usize;

        self.buckets[in_check as usize][from_threated][to_threated]
    }
    ...
    pub fn update_bucket(&mut self, in_check: bool, threats: Bitboard, mv: Move, bonus: i32) {
        let from_threated = threats.contains(mv.from()) as usize;
        let to_threated = threats.contains(mv.to()) as usize;

        let entry = &mut self.buckets[in_check as usize][from_threated][to_threated];
...
impl QuietHistory {
    pub fn get(&self, in_check: bool, threats: Bitboard, stm: Color, mv: Move) -> i32 {
        ...
        (entry.factorizer + entry.bucket(in_check, threats, mv)) as i32
    }
    ...
    pub fn update(&mut self, in_check: bool, threats: Bitboard, stm: Color, mv: Move, bonus: i32) {
        ...
        entry.update_bucket(in_check, threats, mv, bonus);
    }
}
