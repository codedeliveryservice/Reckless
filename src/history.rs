...
type ContinuationHistoryType = [[PieceToHistory<[PieceToHistory<i16>; 2]>; 2]; 2];
type ContinuationCorretionType = [[PieceToHistory<PieceToHistory<i16>>; 2]; 2];
...
pub struct ContinuationCorrectionHistory {
    ...
    entries: Box<ContinuationCorretionType>,
}
...
impl ContinuationHistory {
    ...
    pub fn subtable_ptr(
        &mut self, in_check: bool, capture: bool, piece: Piece, to: Square,
    ) -> *mut [PieceToHistory<i16>; 2] {
        &raw mut self.entries[in_check as usize][capture as usize][piece][to]
    }

    pub fn get(&self, subtable_ptr: *mut [PieceToHistory<i16>; 2], in_check: bool, piece: Piece, to: Square) -> i32 {
        (unsafe { &*subtable_ptr }[in_check as usize][piece][to]) as i32
    }

    pub fn update(&self, subtable_ptr: *mut [PieceToHistory<i16>; 2], in_check: bool, piece: Piece, to: Square, bonus: i32) {
        let entry = &mut unsafe { &mut *subtable_ptr }[in_check as usize][piece][to];
        apply_bonus::<{ Self::MAX_HISTORY }>(entry, bonus);
    }
}
