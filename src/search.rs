...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if bound == Bound::Upper && td.ply >= 1 && depth > 3 {
        ...
        if pcm_move.is_some() && pcm_move.is_quiet() {
            let mut bonus = 128 * depth - 96;
            bonus += (128 * depth - 96) * (depth > 5) as i32;
            bonus += (256 * depth - 192) * (!in_check && best_score <= td.stack[td.ply].static_eval - 128) as i32;
            bonus += (256 * depth - 192) * (best_score <= -td.stack[td.ply - 1].static_eval - 128) as i32;
