...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    let lmp_threshold = if improving || static_eval >= beta + 18 {
        4.2560 + 0.9769 * (initial_depth * initial_depth) as f32
    } else {
        1.7653 + 0.4893 * (initial_depth * initial_depth) as f32
    } as i32;
    ...
    while let Some(mv) = move_picker.next(td, skip_quiets) {
        ...
        if !NODE::ROOT && !is_loss(best_score) {
            ...
            // Futility Pruning (FP)
            let futility_value =
                static_eval + 101 * lmr_depth + 60 * history / 1024 + 88 * (static_eval >= alpha) as i32 + 73;
            ...
            let threshold = if is_quiet {
                -22 * lmr_depth * lmr_depth - 31 * history / 1024 + 18
            } else {
                -103 * depth - 50 * history / 1024 + 46
            };
    ...
    if best_move.is_some() {
        ...
        let malus_quiet = (133 * initial_depth - 50).min(1122) - 36 * quiet_moves.len() as i32;

        let bonus_cont = (99 * depth - 56).min(1265) - 68 * cut_node as i32;
        let malus_cont = (289 * initial_depth - 46).min(1032) - 30 * quiet_moves.len() as i32;
