...
fn correction_value(td: &ThreadData) -> i32 {
    ...
    if td.board.pieces(PieceType::Knight).is_empty() {
        correction += td.no_knight_corrhist[Color::White].get(stm, td.board.no_knight_key(Color::White))
            + td.no_knight_corrhist[Color::Black].get(stm, td.board.no_knight_key(Color::Black));
    }

    if td.board.pieces(PieceType::Bishop).is_empty() {
        correction += td.no_bishop_corrhist[Color::White].get(stm, td.board.no_bishop_key(Color::White))
            + td.no_bishop_corrhist[Color::Black].get(stm, td.board.no_bishop_key(Color::Black));
    }

    if td.board.pieces(PieceType::Rook).is_empty() {
        correction += td.no_rook_corrhist[Color::White].get(stm, td.board.no_rook_key(Color::White))
            + td.no_rook_corrhist[Color::Black].get(stm, td.board.no_rook_key(Color::Black));
    }
...
fn update_correction_histories(td: &mut ThreadData, depth: i32, diff: i32) {
    ...
    if td.board.pieces(PieceType::Knight).is_empty() {
        td.no_knight_corrhist[Color::White].update(stm, td.board.no_knight_key(Color::White), bonus);
        td.no_knight_corrhist[Color::Black].update(stm, td.board.no_knight_key(Color::Black), bonus);
    }

    if td.board.pieces(PieceType::Bishop).is_empty() {
        td.no_bishop_corrhist[Color::White].update(stm, td.board.no_bishop_key(Color::White), bonus);
        td.no_bishop_corrhist[Color::Black].update(stm, td.board.no_bishop_key(Color::Black), bonus);
    }

    if td.board.pieces(PieceType::Rook).is_empty() {
        td.no_rook_corrhist[Color::White].update(stm, td.board.no_rook_key(Color::White), bonus);
        td.no_rook_corrhist[Color::Black].update(stm, td.board.no_rook_key(Color::Black), bonus);
    }
