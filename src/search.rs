...
pub fn start(td: &mut ThreadData, report: Report) {
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        // Aspiration Windows
        if depth >= 2 {
            delta += average * average / 25567;

            alpha = (average - delta).max(-Score::INFINITE);
            beta = (average + delta).min(Score::INFINITE);

            td.optimism[td.board.side_to_move()] = 118 * average / (average.abs() + 247);
...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if let Some(entry) = &entry {
            ...
            }
        {
            if tt_move.is_quiet() && tt_score >= beta {
                let quiet_bonus = (141 * depth - 69).min(1428) + 69 * !cut_node as i32;
                let conthist_bonus = (95 * depth - 64).min(1524) + 64 * !cut_node as i32;
    ...
    {
        let value = 707 * (-(static_eval + td.stack[td.ply - 1].static_eval)) / 128;
        let bonus = value.clamp(-121, 262);
    ...
    if !NODE::ROOT
        && !in_check
        && !excluded
        && td.stack[td.ply - 1].reduction >= 2635
    ...
    if !NODE::ROOT
        ...
        && !excluded
        && depth >= 2
        && td.stack[td.ply - 1].reduction >= 934
        && is_valid(td.stack[td.ply - 1].static_eval)
    ...
    // Razoring
    if !NODE::PV && !in_check && eval < alpha - 316 - 228 * initial_depth * initial_depth {
    ...
    if cut_node
        ...
        && eval >= beta
        && eval >= static_eval
        && static_eval >= beta - 15 * depth + 154 * tt_pv as i32 - 103 * improvement / 1024 + 180
        && td.ply as i32 >= td.nmp_min_ply
        && td.board.has_non_pawns()
        && !potential_singularity
        && !is_loss(beta)
    {
        let r = 5 + depth / 3 + ((eval - beta) / 243).min(3);
    ...
    // ProbCut
    let probcut_beta = beta + 272 - 61 * improving as i32;
    ...
    while let Some(mv) = move_picker.next(td, skip_quiets) {
        ...
        if !improving {
            reduction += (486 - 423 * improvement / 128).min(1272);
        }
        ...
        if !NODE::ROOT && !is_loss(best_score) {
            ...
            skip_quiets |= !in_check
                && move_count
                    >= (4 + initial_depth * initial_depth)
                        / (2 - (improving || static_eval >= beta + 17) as i32);

            // Futility Pruning (FP)
            let futility_value = static_eval
                + 113 * lmr_depth
                + 49 * history / 1024
                + 92 * (static_eval >= alpha) as i32
                + 85;
            ...
            let threshold = if is_quiet {
                -22 * lmr_depth * lmr_depth - 30 * history / 1024 + 17
            } else {
                -109 * depth - 46 * history / 1024 + 46
            };
        ...
        if !NODE::ROOT && !excluded && td.ply < 2 * td.root_depth as usize && mv == tt_move && potential_singularity {
            ...
            if score < singular_beta {
                let double_margin = 2 + 280 * NODE::PV as i32;
                let triple_margin = 70 + 305 * NODE::PV as i32 - 16 * correction_value.abs() / 128;
        ...
        if depth >= 2 && move_count > 1 {
            if is_quiet {
                ...
                reduction += 514;
                reduction -= 105 * history / 1024;
                reduction -= 47 * PIECE_VALUES[td.board.piece_on(mv.to()).piece_type()] / 128;
            }

            reduction -= 3758 * correction_value.abs() / 1024;
            reduction -= 69 * move_count;

            if tt_pv {
                reduction -= 449;
                reduction -= 669 * (is_valid(tt_score) && tt_score > alpha) as i32;
                reduction -= 827 * (is_valid(tt_score) && tt_depth >= depth) as i32;
            }

            if NODE::PV {
                reduction -= 419 + 533 * (beta - alpha > 32 * td.root_delta / 128) as i32;
            }

            if !tt_pv && cut_node {
                reduction += 1687;
                reduction += 1000 * tt_move.is_null() as i32;
            }
            ...
            if score > alpha && new_depth > reduced_depth {
                new_depth += (score > best_score + 41 + 519 * depth / 128) as i32;
                ...
                if new_depth > reduced_depth {
                    ...
                    if mv.is_quiet() && score >= beta {
                        let bonus = (1 + (move_count / depth)) * (159 * depth - 63).min(856);
        ...
        // Full Depth Search (FDS)
            ...
            reduction -= 2831 * correction_value.abs() / 1024;
            reduction -= 51 * move_count;

            if tt_pv {
                reduction -= 767;
                reduction -= 524 * (is_valid(tt_score) && tt_score > alpha) as i32;
                reduction -= 1030 * (is_valid(tt_score) && tt_depth >= depth) as i32;
            }

            if NODE::PV {
                reduction -= 469 + 879 * (beta - alpha > 26 * td.root_delta / 128) as i32;
            }

            if !tt_pv && cut_node {
                reduction += 1337;
                reduction += 1053 * tt_move.is_null() as i32;
            }
    ...
    if best_move.is_some() {
        let bonus_noisy = (128 * depth - 52).min(1047) - 67 * cut_node as i32;
        let malus_noisy = (145 * initial_depth - 66).min(1409) - 24 * noisy_moves.len() as i32;

        let bonus_quiet = (151 * depth - 68).min(1608) - 66 * cut_node as i32;
        let malus_quiet = (128 * initial_depth - 49).min(1090) - 35 * quiet_moves.len() as i32;

        let bonus_cont = (101 * depth - 58).min(1318) - 67 * cut_node as i32;
        let malus_cont = (292 * initial_depth - 44).min(1001) - 31 * quiet_moves.len() as i32;
        ...
        if !NODE::ROOT && td.stack[td.ply - 1].mv.is_quiet() && td.stack[td.ply - 1].move_count == 1 {
            let malus = (86 * initial_depth - 53).min(831);
    ...
    if !NODE::ROOT && bound == Bound::Upper && (!quiet_moves.is_empty() || depth > 3) {
        ...
        if pcm_move.is_quiet() {
            ...
            factor += 244 * (!in_check && best_score <= static_eval.min(raw_eval) - 130) as i32;
            factor += 277
                * (is_valid(td.stack[td.ply - 1].static_eval)
                    && best_score <= -td.stack[td.ply - 1].static_eval - 98) as i32;

            let scaled_bonus = factor * (149 * initial_depth - 43).min(1824) / 128;
            ...
            if td.ply >= 2 {
                ...
                if entry.mv.is_some() {
                    let bonus = (156 * initial_depth - 46).min(1607);
...
fn update_correction_histories(td: &mut ThreadData, depth: i32, diff: i32) {
    let stm = td.board.side_to_move();
    let bonus = (130 * depth * diff / 128).clamp(-4000, 3336);
