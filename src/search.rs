...
pub fn start(td: &mut ThreadData, report: Report) -> SearchResult {
    ...
    let mut score_history = vec![td.best_score; 5];
    let mut score_anchor = 0;
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        let eval_factor = (1.0 + (score_history[score_anchor] - td.best_score) as f32 / 128.0).clamp(0.75, 1.25);

        score_history[score_anchor] = td.best_score;
        score_anchor = (score_anchor + 1).min(4);
