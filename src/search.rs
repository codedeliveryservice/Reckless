...
fn search<NODE: NodeType>(
    ...
    if !NODE::ROOT && !in_check && !excluded && td.stack[ply - 1].mv.is_quiet() && is_valid(td.stack[ply - 1].eval) {
        ...
        td.quiet_history.update(
            ...
            !stm,
            td.stack[ply - 1].mv,
            td.board.is_prior_direct_check(td.stack[ply - 1].piece, td.stack[ply - 1].mv),
            bonus,
        );
    }
    ...
    while let Some(mv) = move_picker.next::<NODE>(td, skip_quiets, ply) {
        ...
        let history = if is_quiet {
            td.quiet_history.get(td.board.all_threats(), stm, mv, td.board.is_direct_check(mv))
    ...
    if best_move.is_present() {
        ...
        if best_move.is_noisy() {
            ...
            for &mv in quiet_moves.iter() {
                td.quiet_history.update(td.board.all_threats(), stm, mv, td.board.is_direct_check(mv), -quiet_malus);
