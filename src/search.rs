...
fn search<NODE: NodeType>(
        ...
        && !tt_move.is_quiet()
    {
        ...
        while let Some(mv) = move_picker.next(td, ply, true) {
    ...
    while let Some(mv) = move_picker.next(td, ply, skip_quiets) {
        if mv == td.stack[ply].excluded || !td.board.is_legal(mv) {
            continue;
        }
        ...
        let history = if is_quiet {
            td.quiet_history.get(td.board.threats(), td.board.side_to_move(), mv)
                + td.conthist(1, mv, ply)
                + td.conthist(2, mv, ply)
        } else {
...
fn qsearch<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, beta: i32, ply: usize) -> i32 {
    ...
    while let Some(mv) = move_picker.next(td, ply, !in_check) {
