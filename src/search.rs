...
pub fn start(td: &mut ThreadData, report: Report) {
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        let multiplier = || {
            ...
            let noisy_factor = if td.root_moves[0].mv.to() == td.board.recapture_square() {
                0.9
            } else if td.root_moves[0].mv.is_noisy() {
                0.95
            } else {
                1.0
            };

            nodes_factor * pv_stability * eval_stability * score_trend * noisy_factor
        };
