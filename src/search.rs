...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if cut_node
        ...
        && eval >= beta
        && eval >= static_eval
        && static_eval >= beta + td.nmp_model.output(depth, tt_pv, improvement)
        ...
        && !is_loss(beta)
    {
        ...
        if score >= beta && !is_win(score) {
            if td.nmp_min_ply > 0 || depth < 16 {
                td.nmp_model.update(depth, tt_pv, improvement, 0.995);
                return score;
            }
            ...
            td.nmp_model.update(depth, tt_pv, improvement, -1.957);
        }
    }
