...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if best_move.is_valid() {
        let score_adjustment = (best_score - beta).clamp(-300, 300);

        let bonus = stat_bonus(depth) + score_adjustment;
        let malus = stat_bonus(depth) + score_adjustment - 16 * (move_count - 1);
