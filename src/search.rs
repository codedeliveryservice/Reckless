...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    while let Some(mv) = move_picker.next(td, skip_quiets) {
        ...
        // Full Depth Search (FDS)
            ...
            reduction -= 3128 * correction_value.abs() / 1024;
            ...
            if tt_pv {
                ...
                reduction -= 875 * cut_node as i32;
            }

            if NODE::PV {
                reduction -= 631 + 603 * (beta - alpha > 34 * td.root_delta / 128) as i32;
            }

            if cut_node {
                reduction += 1253;
                reduction += 986 * tt_move.is_null() as i32;
            }
            ...
            if is_quiet {
                reduction -= 112 * (history - 592) / 1024;
            } else {
                reduction -= 100 * (history - 615) / 1024;
                reduction -= 40 * PIECE_VALUES[td.board.piece_on(mv.to()).piece_type()] / 116;
            }
