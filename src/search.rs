...
fn update_continuation_histories(td: &mut ThreadData, piece: Piece, sq: Square, bonus: i32) {
    if td.ply >= 1 {
        ...
        if entry.mv.is_some() {
            td.continuation_history.update(entry.piece, entry.mv.to(), piece, sq, 1285 * bonus / 1024);
        }
    }
    ...
    if td.ply >= 2 {
        ...
        if entry.mv.is_some() {
            td.continuation_history.update(entry.piece, entry.mv.to(), piece, sq, 1351 * bonus / 1024);
        }
    }
    ...
    if td.ply >= 3 {
        ...
        if entry.mv.is_some() {
            td.continuation_history.update(entry.piece, entry.mv.to(), piece, sq, 872 * bonus / 1024);
        }
    }
    ...
    if td.ply >= 4 {
        ...
        if entry.mv.is_some() {
            td.continuation_history.update(entry.piece, entry.mv.to(), piece, sq, 941 * bonus / 1024);
        }
    }
    ...
    if td.ply >= 5 {
        ...
        if entry.mv.is_some() {
            td.continuation_history.update(entry.piece, entry.mv.to(), piece, sq, 1020 * bonus / 1024);
        }
    }
