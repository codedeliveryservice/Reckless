...
pub fn start(td: &mut ThreadData, report: Report, thread_count: usize) {
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        let multiplier = || {
            ...
            let instability = {
                let delta = score_history.windows(2).rev().take(6).map(|w| (w[1] - w[0]).abs() as f32).sum::<f32>();
                (0.9000 + 0.0100 * (delta / 6.0)).clamp(0.9000, 1.5000)
            };
            ...
            nodes_factor * instability * score_trend * best_move_stability
        };
