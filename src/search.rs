...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    while let Some(mv) = move_picker.next(td, skip_quiets) {
        ...
        if !improving {
            reduction += (r3() - r4() * improvement / 128).min(r5());
        }
        ...
        if depth >= 2 && move_count > 1 + NODE::ROOT as i32 {
            if is_quiet {
                reduction -= v1() * (history - v2()) / 1024;
            } else {
                reduction -= v3() * (history - v4()) / 1024;
                reduction -= v5() * PIECE_VALUES[td.board.piece_on(mv.to()).piece_type()] / 128;
            }

            reduction -= v6() * correction_value.abs() / 1024;
            reduction -= v7() * move_count;
            reduction += v8();

            if tt_pv {
                reduction -= v9();
                reduction -= v10() * (is_valid(tt_score) && tt_score > alpha) as i32;
                reduction -= v11() * (is_valid(tt_score) && tt_depth >= depth) as i32;
                reduction -= v12() * cut_node as i32;
            }

            if NODE::PV {
                reduction -= v13() + v14() * (beta - alpha > v15() * td.root_delta / 128) as i32;
            }
