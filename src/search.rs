...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if cut_node
        ...
        && static_eval
            >= beta - 20 * depth + 128 * tt_pv as i32 + 180
                - td.nmp_history.get(td.board.pawn_key(), td.board.side_to_move(), td.stack[td.ply - 1].mv) / 16
        ...
        && td.board.has_non_pawns()
    {
        ...
        td.nmp_history.update(td.board.pawn_key(), td.board.side_to_move(), td.stack[td.ply - 1].mv, bonus);
