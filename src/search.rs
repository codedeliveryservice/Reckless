...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if best_move.is_some() {
        ...
        let malus_quiet = (134 * initial_depth - 55 - 17 * (move_count - 1) + 200 * skip_quiets as i32).clamp(64, 1273);

        let bonus_cont = (97 * depth - 57).min(1250) - 69 * cut_node as i32;
        let malus_cont = (277 * initial_depth - 49 - 14 * (move_count - 1) + 126 * skip_quiets as i32).clamp(64, 978);
