...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if !in_check
        && td.ply >= 1
        && td.stack[td.ply - 1].reduction >= 3072
    ...
    if best_move.is_some() {
        let bonus = stat_bonus(depth + (best_score > beta + 150) as i32);
        let malus = stat_bonus(depth + (best_score > beta + 150) as i32) - 16 * (move_count - 1);
    ...
    if bound == Bound::Upper && td.ply >= 1 && depth > 3 {
        ...
        let factor = 1
            + (depth > 5) as i32
            + 2 * (!in_check && best_score <= td.stack[td.ply].static_eval - 128) as i32
            + 2 * (td.stack[td.ply - 1].static_eval != Score::NONE
                && best_score <= -td.stack[td.ply - 1].static_eval - 128) as i32;
