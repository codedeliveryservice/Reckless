...
fn search<NODE: NodeType>(
    ...
    {
        update_correction_histories(td, depth, best_score - eval, ply, excluded);
    }
...
fn update_correction_histories(td: &mut ThreadData, depth: i32, diff: i32, ply: isize, excluded: bool) {
    let stm = td.board.side_to_move();
    let corrhist = td.corrhist();

    let bonus = ((142 * depth * diff / 128) / (1 + (excluded && diff < 0) as i32)).clamp(-4923, 3072);
