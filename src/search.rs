...
fn search<NODE: NodeType>(
    ...
    if best_move.is_some() {
        let bonus_noisy = (125 * depth - 57).min(1175)
            - 70 * cut_node as i32
            - 49 * (move_count == 1 + tt_move.is_some() as i32) as i32;
        let malus_noisy = (153 * initial_depth - 64).min(1476) - 24 * noisy_moves.len() as i32;

        let bonus_quiet = (152 * depth - 73).min(1569)
            - 64 * cut_node as i32
            - 49 * (move_count == 1 + tt_move.is_some() as i32) as i32;
        let malus_quiet = (133 * initial_depth - 51).min(1162) - 37 * quiet_moves.len() as i32;

        let bonus_cont = (102 * depth - 56).min(1223)
            - 65 * cut_node as i32
            - 49 * (move_count == 1 + tt_move.is_some() as i32) as i32;
