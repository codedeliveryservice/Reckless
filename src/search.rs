...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if let Some(entry) = entry {
        ...
        {
            if entry.score >= beta && tt_move.is_some() && tt_move.is_quiet() && entry.depth > depth {
                let bonus_quiet = (126 * depth - 75).min(1325);
                td.quiet_history.update(td.board.threats(), td.board.side_to_move(), tt_move, bonus_quiet);
            }
            return entry.score;
        }
    }
