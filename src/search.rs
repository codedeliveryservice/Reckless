...
fn search<const PV: bool>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    if !is_root
        ...
        && td.board.occupancies().len() <= max_cardinality() as usize
    {
        let mut sucesss = 0;
        let code = probe_wdl(&mut td.board, &mut sucesss);

        if sucesss != 0 {
            let (score, bound) = match code {
                2 => (tb_win_in(td.ply), Bound::Lower),
                -2 => (tb_loss_in(td.ply), Bound::Upper),
