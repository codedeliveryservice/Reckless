...
fn search<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, mut beta: i32, depth: i32, cut_node: bool) -> i32 {
    ...
    {
        update_correction_histories(td, depth, best_score - static_eval, had_best_noisy_move);
    }
...
fn update_correction_histories(td: &mut ThreadData, depth: i32, diff: i32, had_best_noisy_move: bool) {
    let stm = td.board.side_to_move();
    let bonus = (138 * depth * diff / 128).clamp(-3964, 3303) / (1 + had_best_noisy_move as i32);
