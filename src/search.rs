...
fn search<NODE: NodeType>(
    ...
    if best_move.is_some() {
        let bonus_noisy = (125 * depth).min(1175) - 70 * cut_node as i32 - 57;
        let malus_noisy = (153 * initial_depth).min(1476) - 24 * noisy_moves.len() as i32 - 64;

        let bonus_quiet = (152 * depth).min(1569) - 64 * cut_node as i32 - 73;
        let malus_quiet = (133 * initial_depth).min(1162) - 37 * quiet_moves.len() as i32 - 51;

        let bonus_cont = (102 * depth).min(1223) - 65 * cut_node as i32 - 56;
        let malus_cont = (306 * initial_depth).min(1018) - 30 * quiet_moves.len() as i32 - 46;
