...
fn search<NODE: NodeType>(
    ...
    if best_move.is_some() {
        let noisy_bonus = ((111 * depth - 54).min(861) - 77 * cut_node as i32).max(0);
        let noisy_malus = ((173 * initial_depth - 53).min(1257) - 23 * noisy_moves.len() as i32).max(0);

        let quiet_bonus = ((179 * depth - 75).min(1335) - 56 * cut_node as i32).max(0);
        let quiet_malus = ((156 * initial_depth - 44).min(1056) - 41 * quiet_moves.len() as i32).max(0);

        let cont_bonus = ((115 * depth - 67).min(972) - 50 * cut_node as i32).max(0);
        let cont_malus = ((343 * initial_depth - 47).min(856) - 21 * quiet_moves.len() as i32).max(0);
