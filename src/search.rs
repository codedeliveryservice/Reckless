...
fn search<NODE: NodeType>(
    ...
    let no_opponent_threats = (td.board.all_threats() & td.board.colors(stm)).is_empty();
    ...
    if cut_node
        ...
        && estimated_score >= beta
        && estimated_score
            >= beta - 8 * depth + 116 * tt_pv as i32
                - 106 * improvement / 1024
                - 34 * no_opponent_threats as i32
