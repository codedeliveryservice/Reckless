...
pub fn start(td: &mut ThreadData, report: Report) {
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        let multiplier = || {
            let nodes_factor = 2.0127 - 1.368 * (td.root_moves[0].nodes as f32 / td.nodes.local() as f32);

            let pv_stability = 1.284 - 0.046 * pv_stability.min(8) as f32;

            let eval_stability = 1.148 - 0.043 * eval_stability.min(8) as f32;

            let score_trend =
                (0.751 + 0.047 * (td.previous_best_score - td.root_moves[0].score) as f32).clamp(0.878, 1.396);
